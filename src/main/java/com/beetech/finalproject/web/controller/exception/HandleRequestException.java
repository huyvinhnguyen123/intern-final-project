package com.beetech.finalproject.web.controller.exception;
...
public class HandleRequestException {
    ...
    @ExceptionHandler({AuthenticationException.class, ExpiredJwtException.class,
            GeneralSecurityException.class, ValidFileExtensionException.class, IOException.class})
     ...
     *
     * @param errorMessage - input error message
     * @return - access denied request body
     */
    @ExceptionHandler({AccessDeniedException.class, AccountException.class})
    public static ResponseEntity<ResponseDto<Object>> handleAccessDeniedException(String errorMessage) {
        ResponseDto<Object> responseDto = ResponseDto.build()
                .withHttpStatus(HttpStatus.FORBIDDEN)
                .withMessage("Access denied: " + errorMessage);
